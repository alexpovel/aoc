...
impl Challenge for Part {
    ...
    fn solve(&self) -> String {
        ...
        for race in races {
            let mut n_ways = 0;
            (1..race.time).for_each(|t_button| {
                let velocity = t_button; // lol nice units bro
                let travel_time = race.time - t_button;
                let distance = velocity * travel_time;

                n_ways += (distance > race.record_distance) as u32;
            });
